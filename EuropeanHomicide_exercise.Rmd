...
ggplot(data = Western_Europe) + 
  ...
  labs(title="Homicides per year", y = "homicides per 100k")
```
...
names(kings)[names(kings)=="Entity"]<- "Country"

kings %>% 
  mutate(duration = `end of reign` - `start of reign`) %>% 
  mutate(midyear = `end of reign` - (`end of reign`-`start of reign`)/2) %>% 
  ggplot(aes(x=midyear, y=duration, fill=midyear)+
  geom_point()+
  geom_smooth()+
  ggtitle("Danish kings duration")
